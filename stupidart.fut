...
module lys_core = {
  type sized_state [h][w] = {rng: rng, paused: bool, score: f32, n_objs: i32,
  ...
  entry init [h][w] (seed: i32) (image_source: [h][w]argb.colour): state =
    ...
    in {rng, paused=false, score, shape=#random, image_source=image_source', image_approx, n_objs=0}
      ...
      if s.paused then s
           ...
           let (image_approx', improved, rng') =
             ...
             case #random -> let (rng, choice) = dist_int.rand (0, 2) s.rng
                             in if choice == 0 then triangle.add s.n_objs s.image_source image_approx rng
                                else if choice == 1 then circle.add s.n_objs s.image_source image_approx rng
                                else rectangle.add s.n_objs s.image_source image_approx rng
             case #triangle ->   triangle.add s.n_objs s.image_source image_approx s.rng
             case #circle ->       circle.add s.n_objs s.image_source image_approx s.rng
             case #rectangle -> rectangle.add s.n_objs s.image_source image_approx s.rng
           in s with image_approx = image_approx'
                with score = s.score - improved
                with rng = rng'
	        with n_objs = s.n_objs + (if improved > 0 then 1 else 0)
